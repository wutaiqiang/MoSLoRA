    ...
    class Linear8bitLt(torch.nn.Module, LoraLayer):
        ...
        def get_delta_weight(self, adapter):
            if self.lora_use_mixer[adapter]:
                return (
                    transpose(
                        self.lora_B[adapter].weight @ self.lora_AB[adapter].weight @ self.lora_A[adapter].weight,
                        False,
                    )
        ...
        def _mixed_batch_forward(
            ...
            for i, active_adapter in enumerate(unique_adapters):
                ...
                if self.lora_use_mixer[active_adapter]:
                    output = lora_B(lora_AB(lora_A(dropout(sub_batch)))) * scaling
                else:
        ...
        def forward(self, x: torch.Tensor, *args, **kwargs) -> torch.Tensor:
            ...
            if self.disable_adapters:
                ...
                for active_adapter in self.active_adapters:
                    ...
                    if not self.use_dora[active_adapter]:
                        if self.lora_use_mixer[active_adapter]:
                            output = lora_B(lora_AB(lora_A(dropout(x)))) * scaling
                        else:
