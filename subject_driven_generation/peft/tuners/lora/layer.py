...
class LoraLayer(BaseTunerLayer):
    ...
    def update_layer(
        self, adapter_name, r, lora_use_mixer, lora_alpha, lora_dropout, init_lora_weights, use_rslora, use_dora: bool = False
    ):
        ...
        #! 
        self.lora_use_mixer[adapter_name] = lora_use_mixer
        if lora_dropout > 0.0:
        ...
        # self.lora_A[adapter_name] = nn.ModuleList([nn.Linear(self.in_features, r, bias=False) for _ in range(expert_A)])
        # self.lora_B[adapter_name] = nn.ModuleList([nn.Linear(r, self.out_features, bias=False) for _ in range(expert_B)])
    ...
    def reset_lora_parameters(self, adapter_name, init_lora_weights):
        ...
        if adapter_name in self.lora_A.keys():
            if init_lora_weights is True:
                ...
                # for idx in range(len(self.lora_A[adapter_name])):
                    # nn.init.kaiming_uniform_(self.lora_A[adapter_name][idx].weight, a=math.sqrt(5))
            elif init_lora_weights.lower() == "gaussian":
                nn.init.normal_(self.lora_A[adapter_name].weight, std=1 / self.r[adapter_name])
                # for idx in range(len(self.lora_A[adapter_name])):
                    # nn.init.normal_(self.lora_A[adapter_name][idx].weight, std=1 / self.r[adapter_name])
            else:
            ...
            # for idx in range(len(self.lora_B[adapter_name])):
    ...
    def _mixed_batch_forward(
        ...
        for i, active_adapter in enumerate(unique_adapters):
            ...
            if self.lora_use_mixer[active_adapter]:
            ...
            if self.lora_use_mixer[active_adapter]:
                lora_output = lora_B(lora_AB(lora_A(dropout(sub_batch)))) * scaling
            else:
...
class Linear(nn.Module, LoraLayer):
    ...
    # def get_weight_lora(self, Weights):
    #     #! Weights dtype: nn.modulelist
    #     res = [l.weight for l in Weights]
    #     scale = 1./math.sqrt(len(res))
    #     return scale*torch.sum(torch.stack(res, dim=-1).to(device=Weights[0].weight.device), dim=-1, keepdim=False)
    
    # def forward_lora(self, Weights, x):
    #     #! Weights dtype: nn.modulelist
    #     #! x: input
    #     res = [l(x) for l in Weights]
    #     scale = 1./math.sqrt(len(res))
    #     return scale*torch.sum(torch.stack(res, dim=-1).to(device=x.device), dim=-1, keepdim=False)
    ...
    def get_delta_weight(self, adapter) -> torch.Tensor:
        ...
        # weight_A = self.get_weight_lora(self.lora_A[adapter])
        # weight_B = self.get_weight_lora(self.lora_B[adapter])
        weight_A = self.lora_A[adapter].weight
        weight_B = self.lora_B[adapter].weight
        if self.lora_use_mixer[adapter]:
            weight_AB = self.lora_AB[adapter].weight

        if cast_to_fp32:
            weight_A = weight_A.float()
            weight_B = weight_B.float()
            if self.lora_use_mixer[adapter]:
                weight_AB = weight_AB.float()

        if self.lora_use_mixer[adapter]:
            output_tensor = transpose(weight_B @ weight_AB @ weight_A, self.fan_in_fan_out) * self.scaling[adapter]
        else:
        ...
        if cast_to_fp32:
            ...
            #! no need to cast back as not modify the lora weigths
            self.lora_A[adapter].weight.data = weight_A.to(dtype)
            self.lora_B[adapter].weight.data = weight_B.to(dtype)
            if self.lora_use_mixer[adapter]:
                self.lora_AB[adapter].weight.data = weight_AB.to(dtype)
        ...
        if self.disable_adapters:
            ...
            for active_adapter in self.active_adapters:
                ...
                if not self.use_dora[active_adapter]:
                    ...
                    # result = result + self.forward_lora(lora_B, self.forward_lora(lora_A, dropout(x))) * scaling
                else:
                    x = dropout(x)
